...
dat <- read.table('./source//K4me3_epic_domain.out.tab', skip = 2, stringsAsFactors = F) %>%
...
p
ggsave('sf4_f1.pdf', p, height = 4, width = 6, device = cairo_pdf, bg = 'transparent')

# K27me3
# visualize deeptools output
dat <- read.table('./source//K27me3_epic_domain.out.tab', skip = 2, stringsAsFactors = F) %>%
  reshape2::melt(id.vars = c("V1", "V2"),
                 value.name = 'y', variable.name = 'x') %>%
  mutate(x = as.numeric(sub('V', '', x)) - 2) %>%
  dplyr::rename(reg = V2, bws = V1) %>% mutate(reg=str_replace(reg, "_peaks.bed", "")) %>% 
...
dat %>%  
  ...
  theme(legend.position = "bottom",
        ...
        plot.margin = margin(0.5, 0.5, 0.5, 0.5, "cm")) ->p
p
ggsave('sf4_f2.pdf', p, height = 3.6, width = 4, device = cairo_pdf, bg = 'transparent')
